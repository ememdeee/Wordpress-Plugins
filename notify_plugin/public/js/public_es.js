;(function($) {
			...
			$.ajax({
				...
				success: function (e) {
					...
					if(e.status == 'success' && showBanner == 'Yes'){
						...
						if (myPerc > 0){
							notify_discount.innerHTML =  "Con <strong>" + myPerc + "%</strong> de descuento";
						...
						// notify_msg.innerHTML = "Tu " + getCustomCookie("cart_type") + " para tu " + newYear + newMake + newModel + (nVin == "CREDIT" ? "" : "(<span style='color:#ffd200;'>" + nVin + "</span>)") + " is ready" + percentageText;
						notify_vin.innerHTML =  "Tu " + planType + " está listo VIN# " + nVin;
						notify_ymm.innerHTML = (!newYear && !newMake && !newModel) ? "" : newYear + newMake + newModel;
						notify_msg.innerHTML = "Obtenga el "+ planType +" para este vehículo. Disfrute del beneficio de la información que le ayudará a tomar la mejor decisión.";
						// notificationBtnText.innerHTML = "Download " + capitalizeFirstLetter(getCustomCookie("cart_type"));
						notificationBtn.innerHTML = "Obtener acceso";
	...
	function notify_cartplan(code,type,preview) {
		...
		if (memberAreaPreview == "cwampv" || memberAreaPreview == "cwampv_proxy") {
			...
			// 	"&source=sticker";
			// }
			// console.log("di IF",memberAreaPreview);
			// debugger;
			if (domainName == 'classicdecoder.com'){
				window.location = 
				redirectUrl + 
